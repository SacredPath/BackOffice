...
BEGIN
    ...
    ON CONFLICT (user_id, currency) DO UPDATE SET
        ...
        updated_at = NOW()
    RETURNING wallet_balances.id, wallet_balances.user_id, wallet_balances.currency, 
              wallet_balances.available, wallet_balances.locked, wallet_balances.total,
              wallet_balances.created_at, wallet_balances.updated_at;
END;
$$;
...
BEGIN
    ...
    ON CONFLICT (user_id, currency) DO UPDATE SET
        ...
        updated_at = NOW()
    RETURNING user_balances.id, user_balances.user_id, user_balances.currency,
              user_balances.amount, user_balances.usd_value,
              user_balances.created_at, user_balances.updated_at;
END;
$$;
